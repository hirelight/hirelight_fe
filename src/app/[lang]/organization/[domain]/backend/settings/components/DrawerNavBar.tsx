...
const DrawerNavBar = ({ onClose, show = false }: IDrawerNavBar) => {
    const backdropRef = React.useRef<HTMLDivElement>(null);
    const drawerRef = React.useRef<HTMLDivElement>(null);

    const closeDrawer = async () => {
        if (backdropRef.current) {
            backdropRef.current.removeAttribute("style");
        }
    ...
    return (
        <div className="relative z-[9999]">
            <motion.div
                ...
                ref={backdropRef}
                className="fixed inset-0 bg-black/20 backdrop-blur-sm dark:bg-slate-900/80 opacity-0 transition-opacity"
            ></motion.div>
            <motion.div
                ref={drawerRef}
                animate={{
